...
| `reward.py` | the after-solve card: an **earned** headline (band lift, hint-free run, a pattern they used to misread), progress to the next readiness number, band counts. Silent when nothing was earned. Flags the milestones that unlock a widget. Read-only | *(prints only)* |
